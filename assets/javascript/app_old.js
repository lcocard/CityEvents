...
function testClicked() {
    ...
    database.ref().on("child_added", function (childSnapshot) {
        ...
        for (var i = 0; i < childSnapshot.val().calEvent.dates.length; i++) {
            if (eventFromDate <= childSnapshot.val().calEvent.dates[i].startDateTime &&
                ...
                var $getEventDate = $("<td>");
                $getEventDate.addClass("getEventDate");
                $(".getEventDate").html(
                    childSnapshot.val().calEvent.dates[i].startDateTime
                );
                $getRow.append($getEventDate);

                var $getEventDesc = $("<td>");
                $getEventDesc.addClass("getEventDesc");
                $(".getEventDesc").html(childSnapshot.val().calEvent.description);
                $getRow.append($getEventDesc);

                var $submitBtn = $(
                    " <td class='getMapButton'> <input type='submit' class='btn btn-success btn-topics-input' value='Submit'/></td>"
                );
                $getRow.append($submitBtn);
